...
/** @walnut_method
 * name: Android Get Object Text
 * description: Read the linked element's text on the Android device and store it in $[textValue]
 * actionType: custom_android_get_object_text
 * context: android
 * needsLocator: true
 * category: Android Device
 */
export async function androidGetObjectText(ctx: WalnutAndroidContext) {
  ...
  if (!outputVar) {
    throw new Error(
      'android_get_object_text: missing output variable — add $[varName] to the step '
  ...
  if (clean === '') {
    ...
    throw new Error(
      'android_get_object_text FAILED: the element resolved but its text is empty '
  ...
  ctx.log('android_get_object_text: "' + clean + '" → ' + outputVar);
