 ...
 * category: iOS Device
 */
export async function iosGetObjectText(ctx: WalnutIosContext) {
  ...
  if (!outputVar) {
    throw new Error(
      'ios_get_object_text: missing output variable — add $[varName] to the step description, '
  ...
  if (clean === '') {
    ...
    throw new Error(
      'ios_get_object_text FAILED: the element resolved but its text is empty.\n'
      + '  locator: ' + ctx.locator + '\n'
  ...
  ctx.log('ios_get_object_text: "' + clean + '" → ' + outputVar);
